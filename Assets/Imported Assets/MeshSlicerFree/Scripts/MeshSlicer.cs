...
public class MeshSlicer
{
    ...
    public async Task<(Mesh, Mesh)> SliceAsync((Vector3,Vector3,Vector3) slicePlane, Mesh targetMesh, Transform targetTransform, bool createSubmeshForIntersection)
    {
        ...
        //        }
        //        Collider[] cols = res.GetComponentsInChildren<Collider>(true);
        ...
        public GameObject CreateSlicedGameObject(
        Mesh slicedMesh,
        GameObject original,
        Material intersectionMaterial)
        {
            // Create empty object
            GameObject res = new GameObject(original.name + "_Sliced");

            // Copy Transform
            res.transform.position = original.transform.position;
            res.transform.rotation = original.transform.rotation;
            res.transform.localScale = original.transform.localScale;
            ...
            original.GetComponent<MeshRenderer>().GetSharedMaterials(mats);

            if (intersectionMaterial != null)
                mats.Add(intersectionMaterial);
    ...
    private Mesh CreateSlicedMesh(MeshVertexDataMapper MVDM, List<int>[] slicedTriangles, bool createSubmeshForIntersection)
    {
        ...
        slicedMesh.RecalculateTangents();
            // GenerateIntersectionUV(slicedMesh);
            slicedMesh.Optimize();

        return slicedMesh;
    }

        private void GenerateIntersectionUV(Mesh mesh)
        {
            ...
            for (int i = 0; i < verts.Length; i++)
            {
                ...
                if (absN.x > absN.y && absN.x > absN.z)
                    uvs[i] = new Vector2(v.y, v.z) * 1f; // X 方向法线 → 用 YZ 平面
                else if (absN.y > absN.x && absN.y > absN.z)
                    uvs[i] = new Vector2(v.x, v.z) * 1f; // Y 方向法线 → 用 XZ 平面
                else
                    uvs[i] = new Vector2(v.x, v.y) * 1f; // Z 方向法线 → 用 XY 平面
            }
